	...
	public class SimpleDynamicsWorld : DynamicsWorld
	{
		...
		public override void StepSimulation(float timeStep, int numSubsteps, float fixedTimeStep)
		{
			...
			if (numManifolds != 0)
			{
				...
				FastList<PersistentManifold> manifolds = (Dispatcher as CollisionDispatcher).Manifolds;
				//int numManifolds = m_dispatcher1.GetNumManifolds();
				ContactSolverInfo infoGlobal = new ContactSolverInfo();
				infoGlobal.TimeStep = timeStep;

				_constraintSolver.SolveGroup(new FastList<CollisionObject>(), manifolds, manifolds.Count, new FastList<TypedConstraint>(), infoGlobal, _debugDrawer);
			}
