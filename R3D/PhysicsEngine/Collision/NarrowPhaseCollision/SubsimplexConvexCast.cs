...
namespace R3D.PhysicsEngine
{
	...
	public class SubsimplexConvexCast : IConvexCast
	{
		...
		public bool CalcTimeOfImpact(Matrix fromA, Matrix toA, Matrix fromB, Matrix toB, CastResult result)
		{
			...
			convex.TransformB = Matrix.CreateFromQuaternion(Quaternion.CreateFromRotationMatrix(rayFromLocalA));
			...
			while ((dist2 > epsilon) && (maxIter-- != 0))
			{
				...
				if (VdotW > 0)
				{
					...
					if (VdotR >= -(MathHelper.Epsilon * LinearMath.MathHelper.Epsilon))
						return false;
					else
					{
