...
namespace R3D
{
	namespace Crypto
	{
		...
		public ref class WSCodeDecrypt
		{
		public:
			WSCodeDecrypt(array<Byte>^ keysrc)
			{
				bf = new Cblowfish();

				pin_ptr<byte> pinnedKeysrc = &keysrc[0];

				byte key[56];
				get_blowfish_key(pinnedKeysrc, key);
			...
			void DecryptHeader(array<Byte>^ encHeader, int& count, int& size)
			{		
				pin_ptr<byte> pinnedEnc = &encHeader[0];
				byte res[8];
				bf->decipher(pinnedEnc, res, 8);
				
				count = (res[1]<<8) | res[0];
				size = (res[5]<<24) | (res[4]<<16) | (res[3]<<8) | res[2];
			}

			void Decipher(array<Byte>^ data, array<Byte>^ out)
			{
				pin_ptr<byte> pinnedData = &data[0];
				pin_ptr<byte> pinnedOut = &out[0];

				bf->decipher(pinnedData, pinnedOut, data->Length);
			}
