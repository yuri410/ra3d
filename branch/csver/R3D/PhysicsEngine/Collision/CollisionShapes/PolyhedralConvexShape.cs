...
namespace R3D.PhysicsEngine
{
	...
	public abstract class PolyhedralConvexShape : ConvexShape
	{
		...
		public override void GetAabb(Matrix trans, out Vector3 aabbMin, out Vector3 aabbMax)
		{
			...
			BulletDebug.Assert(_isLocalAabbValid);

			BulletDebug.Assert(_localAabbMin.X <= _localAabbMax.X);
			BulletDebug.Assert(_localAabbMin.Y <= _localAabbMax.Y);
			BulletDebug.Assert(_localAabbMin.Z <= _localAabbMax.Z);


			Vector3 localHalfExtents = 0.5f * (_localAabbMax - _localAabbMin);
			Vector3 localCenter = 0.5f * (_localAabbMax + _localAabbMin);

			Matrix abs_b = MathHelper.Absolute(trans);

			Vector3 center = MathHelper.MatrixToVector(trans, localCenter);

			Vector3 extent = new Vector3(
				Vector3.Dot(MathHelper.GetRow(abs_b, 0), localHalfExtents),
				Vector3.Dot(MathHelper.GetRow(abs_b, 1), localHalfExtents),
				Vector3.Dot(MathHelper.GetRow(abs_b, 2), localHalfExtents)
										);
		...
		public void RecalcLocalAabb()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				MathHelper.SetValueByIndex(ref vec, i, 1);
				Vector3 tmp = LocalGetSupportingVertex(vec);
				MathHelper.SetValueByIndex(ref _localAabbMax, i, MathHelper.GetValueByIndex(tmp, i) + CollisionMargin);
				MathHelper.SetValueByIndex(ref vec, i, -1);
				tmp = LocalGetSupportingVertex(vec);
				MathHelper.SetValueByIndex(ref _localAabbMin, i, MathHelper.GetValueByIndex(tmp, i) - CollisionMargin);
			}
		}
	}
}
