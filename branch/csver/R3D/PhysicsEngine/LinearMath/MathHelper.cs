...
namespace R3D.PhysicsEngine.LinearMath
{
	public static class MathHelper
	{
		...
		internal static bool TestTriangleAgainstAabb2(Vector3[] vertices, Vector3 aabbMin, Vector3 aabbMax)
		{
			...
			if (Math.Min(MathEx.Min(p1.X, p2.X), p3.X) > aabbMax.X) return false;
		...
		internal unsafe static void SetValueByIndex(ref Vector3 v, int i, float value)
		{
            fixed (Vector3* ptr = &v)
            {
                *((float*)ptr + i) = value;
        ...
        internal unsafe static float GetValueByIndex(Vector3 v, int i)
        {
            return *((float*)&v + i);
            //if (i == 0)
            //    return v.X;
