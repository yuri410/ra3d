...
namespace R3D.PhysicsEngine.Dynamics
{
	...
	public class DiscreteDynamicsWorld : DynamicsWorld
	{
		...
		private FastList<TypedConstraint> _constraints = new FastList<TypedConstraint>();
		...
		private FastList<RaycastVehicle> _vehicles = new FastList<RaycastVehicle>();
		...
		public override void AddConstraint(TypedConstraint constraint, bool disableCollisionBetweenLinkedBodies)
		{
		...
		public void AddRigidBody(RigidBody body, BroadphaseProxy.CollisionFilterGroups group, BroadphaseProxy.CollisionFilterGroups mask)
		{
			if (!body.IsStaticOrKinematicObject)
			{
				body.Gravity = _gravity;
		...
		public void DebugDrawObject(Matrix worldTransform, CollisionShape shape, Vector3 color)
		{
			// Draw a small simplex at the center of the object
			{
				...
				DebugDrawer.DrawLine(start, start + MathHelper.TransformNormal(Vector3.UnitX, worldTransform), Vector3.UnitX);
				DebugDrawer.DrawLine(start, start + MathHelper.TransformNormal(Vector3.UnitY, worldTransform), Vector3.UnitY);
				DebugDrawer.DrawLine(start, start + MathHelper.TransformNormal(Vector3.UnitZ, worldTransform), Vector3.UnitZ);
			}
		...
		protected void SolveConstraints(ContactSolverInfo solverInfo)
		{
			...
			FastList<TypedConstraint> sortedConstraints = new FastList<TypedConstraint>(ConstraintsCount);
			...
			FastList<TypedConstraint> constraintsPtr = ConstraintsCount != 0 ? sortedConstraints : new FastList<TypedConstraint>();
				...
				for (int i = 0; i < _vehicles.Count; i++)
				{
					for (int v = 0; v < _vehicles[i].GetNumWheels(); v++)
					{
						...
						Vector3 axle = new Vector3(
							MathHelper.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 0, _vehicles[i].RightAxis),
							MathHelper.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 1, _vehicles[i].RightAxis),
							MathHelper.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 2, _vehicles[i].RightAxis));
		...
		internal class InplaceSolverIslandCallback : SimulationIslandManager.IIslandCallback
		{
			...
			private IDebugDraw _debugDrawer;
			private FastList<TypedConstraint> _sortedConstraints;

			public InplaceSolverIslandCallback(
				ContactSolverInfo solverInfo,
				IConstraintSolver solver,
				FastList<TypedConstraint> sortedConstraints,
            ...
            {
                get { return _solverInfo; }
                set { _solverInfo = value; }
            }
            public IConstraintSolver Solver
            {
                get { return _solver; }
                set { _solver = value; }
            }
            public FastList<TypedConstraint> Constraints
            {
                get { return _sortedConstraints; }
                set { _sortedConstraints = value; }
            }
            public IDebugDraw DebugDrawer
            {
                get { return _debugDrawer; }
                set { _debugDrawer = value; }
            }

			public void ProcessIsland(FastList<CollisionObject> bodies, FastList<PersistentManifold> manifolds, int numManifolds, int islandID)
			{
				//also add all non-contact constraints/joints for this island
				FastList<TypedConstraint> startConstraint = new FastList<TypedConstraint>();
