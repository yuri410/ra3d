...
namespace R3D.PhysicsEngine
{
	...
	public class GjkPairDetector : DiscreteCollisionDetectorInterface
	{
		...
		public override void GetClosestPoints(DiscreteCollisionDetectorInterface.ClosestPointInput input, DiscreteCollisionDetectorInterface.Result output, IDebugDraw debugDraw)
		{
            ...
            Vector3 pointOnA, pointOnB = new Vector3();
...

			{
				...
				while (true)
				{
					...
					{
#if DEBUG
                        R3DConsole.Instance.Write("GjkPairDetector maxIter exceeded: {0}", _currentIteration.ToString());
                        R3DConsole.Instance.Write("sepAxis=({0},{1},{2}), squaredDistance = {3}, shapeTypeA={4}, shapeTypeB={5}",
							_cachedSeparatingAxis.X.ToString(),
                            _cachedSeparatingAxis.Y.ToString(),
                            _cachedSeparatingAxis.Z.ToString(),
                            squaredDistance.ToString(),
                            _minkowskiA.ShapeType.ToString(),
                            _minkowskiB.ShapeType.ToString()
						);
#endif
						break;
					}
