...
{
	[Serializable]
    public class TriangleMesh : StridingMeshInterface
    {
        private int _numTriangles;
        private FastList<Vector3> _verts;

        public TriangleMesh()
        {
            _numTriangles = 0;
            _verts = new FastList<Vector3>();
        }
        ...
        {
            verts = new FastList<Vector3>();
            for (int i = 0; i < 3; i++)
            {
                verts.Add(_verts[subpart * 3 + i]);
            }
            indicies = new FastList<int>();
            indicies.Add(0);
            indicies.Add(1);
            indicies.Add(2);
            numfaces = 1;
